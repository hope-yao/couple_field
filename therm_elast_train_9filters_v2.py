...
class FEA_Net_h():
    ...
    def apply_physics_constrain(self):
        ...
        self.wyy_np = np.zeros_like(self.wyy_ref) #* 0.9
        self.wxy_np = np.zeros_like(self.wxy_ref) #* 1.9
        self.wyx_np = np.zeros_like(self.wyx_ref) #* 1.9
        ...
        self.trainable_var_ref = np.concatenate([self.wtx_ref.flatten(),
                                                self.wty_ref.flatten(),
                                                self.wxt_ref.flatten(),
        ...
        self.trainable_var_pl = tf.placeholder(tf.float64, shape=(9 * 9,), name='filter_vector')

        wtx_tf, wty_tf, wxt_tf, wyt_tf, wxx_tf, wyy_tf, wxy_tf, wyx_tf, wtt_tf = tf.split(self.trainable_var_pl,9)
